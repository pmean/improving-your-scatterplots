...
simple_framework <- ggplot(
  ...
  xlab("House living space (thousands of square feet)") +
...
### Size: area versus diameter?

```{r area-versus-diameter}
area_chart <- 
  ggplot(sa, aes(Size, Price)) + 
  xlab("House living space (thousands of square feet)") +
  ylab("House price (thousands of dollars)") +
  geom_point(shape=0, size=sa$Bedrooms) +
  theme(legend.position="none")
ggsave(
  "../images/area-chart.png", 
  area_chart, width=4, height=4)
tmp <- sa$Bedrooms^2/4
radius_chart <- 
  ggplot(sa, aes(Size, Price)) + 
  xlab("House living space (thousands of square feet)") +
  ylab("House price (thousands of dollars)") +
  geom_point(shape=024/34, size=tmp) +
  theme(legend.position="none")
ggsave(
  "../images/radius-chart.png", 
  radius_chart, width=4, height=4)
```
...
![Plot showing size as bedrooms and shape as fireplace](../images/size-and-shape.png)
...
### Always pair color with size or shape
+ Avoids problems reproduction
  + Poor quality
  + Black and white
+ Helps with patients having color blindness
